...
def read_test_answers(path, test_name):
    ...
    # Convert studentID to integer and then to string to remove '.0'
...
def main():
    ...
    for index, student in student_details_df.iterrows():
        ...
        if not student_edustat_data.empty:
            edustat_row = student_edustat_data.iloc[0]
            q3_val = edustat_row.get('Q3')
            subjects = [item.strip() for item in q3_val.split(',')] if isinstance(q3_val, str) and q3_val else []

            for col in student_edustat_data.columns:
                if not col.startswith('Q') or not col[1:].isdigit():
                    continue

                q_num_str = col[1:]
                raw_val = edustat_row.get(col)
                ...
                q_num = int(q_num_str)
                formatted_q_num = f"{q_num:02d}"
                answer_key = f"e_{formatted_q_num}"
                answer_value = val

                if q_num == 3:
                    answer_value = subjects
                elif q_num == 4:
                    answer_value = {}
                    subject_scores = {}
                elif q_num == 6:
                    answer_value = [item.strip() for item in val.split(',')] if isinstance(val, str) else []
                    extracurriculars = answer_value
                elif q_num == 7:
                    answer_value = [item.strip() for item in val.split(',')] if isinstance(val, str) else []
                    parent_careers = answer_value
                elif q_num == 9:
                    study_abroad_preference = val
                elif q_num == 11:
                    work_style_preference = val
                
                combined_answers[answer_key] = answer_value
